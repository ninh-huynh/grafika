...
public class ExtractDecodeEditEncodeMuxVideo {
    private static final String TAG = ExtractDecodeEditEncodeMuxVideo.class.getSimpleName();
    private static final boolean VERBOSE = false; // lots of logging
    static final String mInpPrefix = /*WorkDir.getMediaDirString()*/ "";
    /** How long to wait for the next buffer to become available. */
    private static final int TIMEOUT_USEC = 10000;
    /** Where to output the test files. */
    private File OUTPUT_FILENAME_DIR /*= Environment.getExternalStorageDirectory()*/;
    ...
    public static class TestWrapper implements Runnable {
        ...
        private ExtractDecodeEditEncodeMuxVideo mTest;
        private TestWrapper(ExtractDecodeEditEncodeMuxVideo test) {
            mTest = test;
        }
        @Override
        public void run() {
            ...
            try {
                ...
                Log.i(TAG, "transcode took " + (endTime - startTime) + " (ms)");
            }
        }
        /**
         * Entry point.
         */
        public static void runTest(ExtractDecodeEditEncodeMuxVideo test) throws Throwable {
//            test.setOutputFile();
     ...
     */
    public void setVerifyAudioFormat() {
    ...
    public void setOutputDir(File dir) {
        this.OUTPUT_FILENAME_DIR = dir;
    }
    /**
     * Sets the name of the output file based on the other parameters.
     *
     * <p>Must be called after {@link #setSize(int, int)} and {@link #setSource(Uri)}.
     */
...
//        sb.append(getClass().getSimpleName());
        assert mSourceUri != null : "should have called setSource() first";
//        sb.append('-');
        sb.append(System.currentTimeMillis());
        if (mCopyVideo) {
            assert mWidth != -1 : "should have called setSize() first";
            assert mHeight != -1 : "should have called setSize() first";
            sb.append('-');
            sb.append("video");
            sb.append('-');
            sb.append(mWidth);
...
//        mOutputFile = sb.toString();
    ...
    public void setOutputVideoMimeType(String mimeType) {
    ...
    private void extractDecodeEditEncodeMux() throws Exception {
        ...
        try {
            if (mCopyVideo) {
                ...
                assert videoInputTrack != -1 : "missing video track in test video";
            ...
            if (mCopyAudio) {
                ...
                assert audioInputTrack != -1 : "missing audio track in test video";
    ...
    protected ParcelFileDescriptor getAssetFileDescriptorFor(final Uri uri)
    ...
    private MediaExtractor createExtractor() throws IOException {
        ...
        ParcelFileDescriptor srcFd = getAssetFileDescriptorFor(mSourceUri);
        extractor = new MediaExtractor();
        extractor.setDataSource(srcFd.getFileDescriptor(), 0, srcFd.getStatSize());
...
//                        fail("video encoder changed its output format again?");
                ...
                assert muxing: "should have added track before processing output";
                if (VERBOSE) {
...
//                        fail("audio encoder changed its output format again?");
        ...
        if (mCopyVideo) {
            assert videoDecodedFrameCount == videoEncodedFrameCount : "encoded and decoded video frame counts should match";
            assert videoDecodedFrameCount <= videoExtractedFrameCount : "decoded frame count should be less than extracted frame count";
        }
        if (mCopyAudio) {
            assert pendingAudioDecoderOutputBufferIndex == -1 : "no frame should be pending";
        }
    }
